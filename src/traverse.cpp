...
public:
  ...
  void visit(ast::Enum &num) override {
    auto *enumSym = insert<sym::EnumType>(man.cur(), log, num);
    enumSym->loc = num.loc;
    ...
    for (const ast::EnumCase &cs : num.cases) {
      ...
      insert(man.cur(), log, sym::Name(cs.name), std::move(caseSym));
    }
    man.leaveScope();
  }
